...
// Define routes
const productRoute = require("./routes/Product.Routes"); // Import Product-related routes
app.use("/products", productRoute); // Use the Product router for requests to /products route

const cartRoute = require("./routes/Cart.Routes"); // Import Product-related routes
app.use("/carts", cartRoute); // Use the Product router for requests to /products route

// Define a simple root endpoint
app.get("/", (req, res) => {
  res.setHeader("Content-type", "text/html");
  res.send(`<h1>Space Product Authentication</h1>`); // Send a basic HTML response for the root endpoint
});
